...
const postId = process.env.AUTH_POST;
...
app.get('/auth', (req, res) => {
    ...
    res.render('auth', { redirect, appName: name, post: `https://wasteof.money/posts/${postId}` });
});

app.get('/demo', (req, res) => {
    const redirectLocation = new Buffer(`http://${req.hostname}:${port}/demo/handle`).toString('base64');
    res.redirect(`http://${req.hostname}:${port}/auth?redirect=${redirectLocation}&name=demo`);
});

app.get('/demo/handle', async (req, res) => {
    const { priv } = req.query;
    const { data } = await axios.get(`http://${req.hostname}:${port}/api/verifyToken?priv=${priv}`);
    if(data.error) {
        return res.json({ error: true, info: data.info });
    } else {
        return res.json({ error: false, info: "it worked!", username: data.username });
    }
});
...
app.get('/api/done', async (req, res) => {
    ...
    const { data } = await axios.get(`https://api.wasteof.money/posts/${postId}/comments`);
